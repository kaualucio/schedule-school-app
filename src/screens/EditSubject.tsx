import {  Heading, HStack, Input, KeyboardAvoidingView, ScrollView, Spinner, useTheme, VStack } from 'native-base';
...
export function EditSubject() {
  ...
  const subjectItem: ISubject = route.params["subject"]
  const { handleRefetchData } = useRefetch()
  const [daysSubjectInAWeek, setDaysSubjectInAWeek] = useState<string[]>(JSON.parse(subjectItem.classesInAWeek))
  const [subject, setSubject] = useState(subjectItem.name)
  const [professor, setProfessor] = useState(subjectItem.professor)
  const [tagName, setTagName] = useState(subjectItem.tagName)
  const [average, setAverage] = useState(subjectItem.average)
  ...
  async function handleAddSubject() {
    ...
    try {
      const response = await Subject.update(subjectItem.id, { name: subject, professor, average, classesInAWeek: JSON.stringify(daysSubjectInAWeek), description, tagName })
  ...
  return (
    <TouchableWithoutFeedback onPress={() => Keyboard.dismiss()}>
      <KeyboardAvoidingView flex={1}  bg="black.700" behavior={Platform.OS === "ios" ? "padding" : "height"}>
        ...
        >
          <Header icon={Book} button add handleFunction={handleAddSubject} title="Editar matéria" />
            <VStack px={4}>
              ...
              <Title title="Horário*" icon={CalendarBlank} />
                <HStack justifyContent="space-between" alignItems="center" mt={3} mb={4}>
                  {
                    weekDays.map((day) => (
                      <Checkbox key={day} daysSubjectInAWeek={daysSubjectInAWeek} handleScheduleSubject={handleScheduleSubject} label={day} />
                    ))
                  }
