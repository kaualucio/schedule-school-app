...
export function EditHomework() {
  ...
  const scheduleItem = route.params["homework"]
  const { handleRefetchData } = useRefetch()
  const [subjects, setSubjects] = useState<ISubject[]>([])
  const [date, setDate] = useState<Date | null>(new Date((scheduleItem.deadline_date * 1000)));
  const [mode, setMode] = useState<any>('date');
  const [type, setType] = useState(scheduleItem.category);
  const [subject, setSubject] = useState(scheduleItem.subject_tagName);
  ...
  async function handleEditHomework() {
      
      if(!subject || !date || !title || !description) {
        return Alert.alert('Campos inválidos', 'Os campos em branco não são permitidos, por favor preencha-os para continuar.')
      }
      try {
        const response = await HomeworkModel.update(scheduleItem.id, { title, category: type, subject_tagName: subject, description, deadline_date: date })
      ...
      <KeyboardAvoidingView flex={1}  bg="black.700" behavior={Platform.OS === "ios" ? "padding" : "height"}>
        <TouchableWithoutFeedback onPress={() => Keyboard.dismiss()}>
          <ScrollView 
            ...
            <Header icon={NotePencil} button add title="Editar anotação" handleFunction={handleEditHomework}  />
            <VStack px={4}>
